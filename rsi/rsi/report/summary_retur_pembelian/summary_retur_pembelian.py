...
def execute(filters=None):
	...
	data = frappe.db.sql("""select a.posting_date,date_format(a.posting_date,"%W"),count(1) as "qty" , sum(a.base_grand_total) as "omset" 
		...
		where a.docstatus=1 and a.is_return=1 {} and (a.posting_date between "{}" and "{}") a group by a.posting_date """.format(where,filters.get("from_date"),filters.get("to_date")),as_list=1)
	return columns, data